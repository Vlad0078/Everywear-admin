...
const Add: React.FC<AddProps> = ({ token }) => {
  ...
  const onSubmitHandler: FormEventHandler = async (event) => {
    ...
    try {
      ...
      if (useVton && vtonImage && vtonCategory) {
      ...
      if (response) {
        ...
        setImages([]);
        setImagesUrl([]);
        setSizeOptions((state) =>
          produce(state, (draft) => {
            draft.forEach((size) => (size.active = false));
          })
        );
        setColors((state) =>
          produce(state, (draft) => {
            draft.forEach((color) => (color.active = false));
          })
        );
        setVtonCategory(undefined);
        setVtonImage(null);
